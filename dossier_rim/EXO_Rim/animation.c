...
int main(int argc, char ** argv) {
    ...
    if (SDL_Init(SDL_INIT_VIDEO) != 0) {
        end_sdl(0, "Erreur d'initialisation de la SDL", NULL, NULL);
    }

    if (!(IMG_Init(IMG_INIT_PNG) & IMG_INIT_PNG)) {
        end_sdl(0, "Erreur d'initialisation de SDL_image", NULL, NULL);
    }
    ...
    SDL_Surface * background_image = IMG_Load("arrierePlan.png");
    if (!background_image) {
        end_sdl(0, "Erreur chargement image arrière-plan", window, renderer);
    }
    ...
    if (!image) {
        end_sdl(0, "Erreur chargement image personnage", window, renderer);
    }
